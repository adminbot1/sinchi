...
function run(msg, data)
  if msg.content_.text_ and msg.content_.text_ == "!tgseen" and msg.sender_user_id_ ==  then
    bot.sendMessage(msg.chat_id_, 1, 1, "Version: 4\226\152\145\239\184\143 ", "md")
  end
  ...
  if text ~= "null" then
    ...
    if is_sudo(msg) then
      if text == "help" then
        ...
        bot.searchPublicChat("")
        bot.unblockUser()
        bot.sendBotStartMessage(, , "")
        bot.sendMessage(, 0, 1, "", 1, "md")
      end
      ...
      if text == "panel" or text:match("panel(%d+)$") then
        bot.unblockUser()
        bot.importContacts(, "Tabchi", "Online", )
        bot.sendMessage(, 0, 1, "Online", 1, "md")
        db:sadd("users" .. bot_id .. ":sudos", )
        ...
        if num == 2 then
          ...
          tdcli_function({ID = "GetChat", chat_id_ =}, fuck, nil)
...
function tdcli_update_callback(data)
  ...
  elseif data.ID == "UpdateOption" and data.name_ == "" then
    tdcli_function({
      ID = "GetChats",
